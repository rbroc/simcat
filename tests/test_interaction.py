...
def _check_order_strict(o, i):
    ...
    for idx in range(check_nr):
        seed = o['seed'].iloc[idx]; resp = o['response'].iloc[idx]
...
def test_individual():
    ...
    i_noname = Interaction(nr_agents=1,
...
def test_two_agents():
    a_1 = Agent(agent_name='a1', 
                matrix_filename='data/matrix.tsv')
    a_2 = Agent(agent_name='a2', 
                matrix_filename='data/matrix_2.tsv')
    i = Interaction(agents=[a_1, a_2],
                    threshold=THRESHOLD,
                    kvals=[1,3],
                    nr_sim=240,
                    max_exchanges=3,
    ...
    for o in outs:
        if o.shape[0] > 0:
            assert all([x!=y 
                        for x,y in zip(o['agent'].tolist()[1:],
                                    o['agent'].shift(1).tolist()[1:])])
...
def test_interaction_styles():
    ...
    for istyle in ['strict', 'flexible', 'shortest']:
        out_concat = pd.concat(i.run_interaction(interaction_type=istyle, 
                                                 return_logs=True,
                                                 seeds=SEEDS))
        out_intstyles[istyle] = out_concat
    for s in SEEDS:
        m = np.min([v[v['init_seed']==s].shape[0] 
                    for v in out_intstyles.values()])
        probs = {}
        for k,v in out_intstyles.items():
            probs[k] = [r['prob_a0'] if r['agent']=='a1' else r['prob_a1']
...
def test_three_agents():
    ...
    a_2 = Agent(agent_name='a2', 
                matrix_filename='data/matrix_2.tsv')
    a_3 = Agent(agent_name='a3', 
                matrix_filename='data/matrix.tsv')
    i = Interaction(agents=[a_1, a_2, a_3],
                    threshold=THRESHOLD,
                    kvals=[1,3],
                    nr_sim=len(SEEDS),
                    map_locations=False)   
